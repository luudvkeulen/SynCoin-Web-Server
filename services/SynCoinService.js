...
/**
 * @param {string} web3Address string Address of the web3-enabled node.
 * @param {string|{address: string, privateKey: string}} walletCreationAccount Unencrypted account or private key with funds to be used for customer wallet creation.
 * @param {string} shopContractAddress Address of the deployed shop contract.
 * @param {string|Number} syncoinRate
 */
module.exports = function SynCoinService(web3Address, walletCreationAccount, shopContractAddress, syncoinRate) {
     ...
     *
     * @param {Number|string} syncoin
     * @returns {string} Amount of wei, wrapped in a string so no precision is lost.
     */
    function syncoinToWei(syncoin) {
        return new BigNumber(syncoin).times(syncoinRate).times('1e+18').ceil().toFixed();
    }

    /**
     * Converts wei to SynCoin.
     *
     * @param {Number|string} wei
     * @returns {string} Amount of SynCoin, wrapped in a string so no precision is lost.
     */
    function weiToSyncoin(wei) {
        return new BigNumber(wei).dividedBy(syncoinRate).dividedBy('1e+18').toFixed();
    }
     ...
     * @param {Number} amount In SynCoin.
    ...
    async function sendTransaction(walletAddress, encryptedAccount, password, toAddress, amount, data) {
        ...
        let sendMethod = walletContract.methods.send(toAddress, syncoinToWei(amount), data);
    ...
    async function getWalletTransactions(walletAddress) {
        ...
        return Promise.all(events.map(async (event) => {
            ...
            return new WalletTransaction(
                ...
                weiToSyncoin(sent ? -event.returnValues.amount : event.returnValues.amount),
    ...
    /**
     * @param {string} address
     * @returns {Promise|string} Resolves when the balance is received.
     */
    async function getBalance(address) {
        return weiToSyncoin(await web3.eth.getBalance(address));
    }
